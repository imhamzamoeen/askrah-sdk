...
__export(server_exports, {
  createRefHandler: () => createRefHandler,
  getRefFromRequest: () => getRefFromRequest
});
...
async function signCookieValue(payload, secret) {
  ...
  const encodedPayload = import_jose.base64url.encode(jsonBytes);
  ...
  const encodedSignature = import_jose.base64url.encode(new Uint8Array(signatureBuffer));
...
async function verifyCookieValue(cookieValue, secret, maxAgeSeconds) {
  ...
  let signatureBytes;
  try {
    signatureBytes = import_jose.base64url.decode(encodedSignature);
  } catch {
    return null;
  }
  const key = await importKey(secret);
  ...
  let payload;
  try {
    const jsonBytes = import_jose.base64url.decode(encodedPayload);
